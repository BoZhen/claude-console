...
- **流式回答** —— 回复边写边显示。CLI 是成串爆发式输出的，所以文本经过一个小的
  ...
  句子中途不会重排。出字过程中向上滚动，视图就停在你放的位置 —— 跟随尾部只到你离开
  为止，回到底部又会重新跟随。
- **工具调用折叠** —— 连续调用*同一个*工具会合并成一行
  （`▶ Bash ×5  ruff check .  1 failed`），展开即可看到各张卡片。中间只要插入
  任何别的东西 —— 一段回答、一次编辑、一次审批 —— 这一串就结束。
- **Plan 悬停区** —— 钉在对话上方的任务列表，显示当前步骤、进度计数，并可折叠到只剩
  当前任务；最后一步完成几秒后自行退场。背后的工具由控制台自己带进 session（CLI 自带
  的任务列表随版本时有时无），并提醒模型随时更新，所以计划不会停在做了一半的样子。
...
- **模型** —— 一个 `🤖` 药丸显示当前 session 的模型，点一下即可切换（下一轮生效，
  不重启）。某一条消息也可以单独走别的模型：以 `@haiku …`、`@sonnet …`、`@opus …`
  或 `@fable …` 开头，这一轮就用那个模型，结束后自动切回。简单问题不必付这个 session
  的价钱，也不用记着切回来。输入 `@` 会弹出补全列表，标明每个写法解析到哪个模型
  （↑/↓ 选择，Tab 或 Enter 填入，Esc 关闭）。模型 id 的任意片段都行，且与实时模型列表
  匹配，所以一年以后 `@fable` 仍然指最新的 fable；有长窗口 `[1m]` 变体的优先用长窗口
  —— 这类消息通常是在长 session 里发的，普通窗口只会被拒。完整 id 按原样使用，这就是
  要普通窗口的写法。`@` 后面不是模型名的，原样作为文本发送，并给出提示。
- **思考深度** —— 一个 `🧠` 药丸（low / medium / high / xhigh / max）用于设定推理
  深度，可随时切换：`/effort` 直接发给运行中的 session，不重启、不重放历史。还有第六
  档 **ultracode**，以 xhigh 运行，并让每条消息都启用多代理编排。
...
- 语音图标来自 Microsoft 的 [Fluent Emoji](https://github.com/microsoft/fluentui-emoji)
  （MIT），位于 `static/icons/`。
- 页面带着自己被服务时的代码哈希，服务端报告的哈希不同就自行重新加载，因此打开着的
  标签页不会拿昨天的脚本跑今天的服务端。重新加载前会先保存草稿。
